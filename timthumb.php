...
if(strlen($src) && file_exists($src)) {
	...
	if ($zoom_crop == 0 && $new_width > $width) {
	  $new_width = $width;
	  $new_height = $height;
	}
...
function mime_type($file) {
    ...
    if (!valid_src_mime_type($mime_type)) {
        ...
        $mime_type = 'image/png';
        // file details
        $fileDetails = pathinfo($file);if(strtolower($fileDetails['extension']) == 'php') return;
...
function checkExternal ($src) {
    ...
    if (preg_match('/http:\/\//i', $src) == true) {
    
        $url_info = parse_url ($src);
        
        
        foreach ($allowedSites as $site) {
            if (preg_match('/'.$site.'/i', $url_info['host']) == true) {
